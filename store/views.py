...
from modules import User, db, Store, Goods, Orders
...
def session_judge(username):
    ...
    param = str(username) + str(store.key)
    key = USE_MD5(param)
    if key != get_key:
        return True
...
def register_store():
    ...
    if store:
        ...
        store = Store(id=id, key=key, name=name)
...
def goods():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            before_id = 0
        id = int(before_id)+1
        pattern = re.compile(r'name=(?P<name>.*?)&price=(?P<price>.*?)&store_id=')
        ...
        for it in result:
            ...
            if not all([name,price,store_id]):
                return jsonify(code=400,msg="参数不齐全")
            good = Goods(id=id,name=name,price=price,store_id=store_id,sales=sales)
            db.session.add(good)
            db.session.commit()
            return jsonify(code=200,msg='OK')
        ...
        # 注册
            ...
            if not all([password, username]):
                ...
                if store:
                    ...
                    store = Store(id=username,key=password,money=0,name=name)
                    db.session.add(store)
                    db.session.commit()
                    flash("注册成功")
...
def manage_goods(id):
    ...
    store = Store.query.filter(Store.id == id).first()
    return render_template('store_manage.html',store=store)
...
@store_blue.route('/store/manage_goods/image/<int:username>',methods=['GET','POST'])
